...
export default function Nav(): JSX.Element {
      ...
      <nav className="z-[100] sticky top-0 left-0 dark:bg-bg-dark-1000 bg-white grow shadow-grey flex flex-col justify-between rounded-b-xl">
        <div>
          <BaseNavItems name={loaderData.name}>
            <button
              className="absolute left-0 top-0 p-6 text-3xl"
              onClick={closeNav}
            >
              x
            </button>
  ...
  return (
    <nav
      id="nav"
      className={`sticky top-0 dark:bg-bg-dark-1000 w-[246px] ${
        isDesktopL ? "h-[1024px]" : "h-[600px]"
      } max-h-screen shrink-0 shadow-grey flex flex-col justify-between`}
    >
      <div>
        <BaseNavItems name={loaderData.name} />
      </div>
...
function BaseNavItems({ name, children }: BaseNavItemsProps) {
  ...
  useEffect(() => {
    if (
      location.pathname === "/managementLogin" ||
      location.pathname === "/managementMasterCommittee"
    ) {
  ...
  return (
    <>
      <div className="flex items-center flex-col border-text-white dark:border-border-light">
        ...
        <div className="py-6 dark:text-sky-300">{name}</div>
      </div>
      <NavItem Image={HouseImage} text="Home" className="pt-2" page="home" />
      <NavItem
        Image={CheckerImage}
        text="Confirmation Checker"
        page="checker"
      />
      ...
      <div
        className={`pl-7 overflow-hidden ${
          isManagementGroupOpen ? "h-32 transition-height duration-300" : "h-0"
        }`}
      >
        <div className="border-l-4 border-text-dark-400/50">
          <NavItem
            Image={ManagementLoginImage}
            text="Login"
            page="managementLogin"
          />
          <NavItem
            Image={ManagementMasterCommitteeImage}
            text="Master List"
