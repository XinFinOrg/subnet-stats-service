...
export class GrandMasterManager {
  ...
  constructor() {
    if (!(window as any).ethereum) {
      throw new ManagerError(
        "XDC Pay Not Installed",
        ErrorTypes.WALLET_NOT_INSTALLED
      );
    }
  ...
  async login(): Promise<AccountDetails> {
    try {
      const { rpcUrl, denom } =
        await this.statsServiceClient.getChainSettingInfo();
      const { accountAddress, balance, minimumDelegation, networkId } =
        await this.grandMasterAccountDetails();
      return {
        accountAddress,
        balance,
        rpcUrl,
  ...
  async addNewMasterNode(address: string): Promise<void> {
    try {
      ...
      const nonce = await this.web3Client!.eth.getTransactionCount(
        accountAddress,
        undefined,
    ...
    try {
      ...
      if (capValue > 0) {
        ...
        await this.web3Contract.methods
          .unvote(replaceXdcWith0x(address), Math.abs(capValue))
          .send({
            from: accountAddress,
            nonce,
            value: "0x0",
  ...
  async onAccountChange(changeHandler: (accounts: string) => any) {
    (this.web3Client!.currentProvider as any).on(
      "accountsChanged",
      (accounts: string[]) => {
  ...
  async getCandidates() {
    try {
      ...
      throw new ManagerError(
        "Unable to get list of candidates",
  ...
  private async getCandidates_temporary() {
    ...
    if (!result) {
      throw new ManagerError(
        "Fail to get list of candidates from xdc subnet, empty value returned"
      );
    }
    const { candidates, success, epoch } = result;
    if (epoch == 0) {
      throw new ManagerError("Now in the first epoch, please wait");
    }
  ...
  private async getGrandmasterAddressAndMinimumDelegation(
    forceRefreshGrandMaster?: boolean
  ): Promise<{ minimumDelegation: number; grandMasterAddress: string[] }> {
    try {
      if (!this.grandMasterInfo || forceRefreshGrandMaster) {
        const grandMasterResult = await this.web3Contract.methods
          .getGrandMasters()
          .call();
        if (!grandMasterResult || !grandMasterResult.length)
          throw new ManagerError(
            "No grand master found in the node",
            ErrorTypes.INTERNAL_ERROR
          );
        let minimumDelegation = 0;
        const minimumDelegationResult: number[] =
          await this.web3Contract.methods.minCandidateCap().call();
      ...
      console.log(error);
      throw new ManagerError(
        "Error while fetching grand master related data from the node",
        ErrorTypes.INTERNAL_ERROR
      );
    }
  }
  ...
  private async grandMasterAccountDetails() {
    ...
    if (!accounts || !accounts.length || !accounts[0].length) {
      throw new ManagerError(
        "No wallet address found, have you logged in?",
        ErrorTypes.WALLET_NOT_LOGIN
      );
    }
    const accountAddress = accounts[0];
    const balance = await this.web3Client!.eth.getBalance(
      accountAddress,
      undefined,
      { number: FMT_NUMBER.NUMBER, bytes: FMT_BYTES.HEX }
    );
    ...
    const { grandMasterAddress, minimumDelegation } =
      await this.getGrandmasterAddressAndMinimumDelegation();
    ...
    switch (error.code) {
      ...
      case 405:
        return new ManagerError(
          error.message,
          ErrorTypes.REVERTED_WITH_NO_REASON
        );
      default:
        return new ManagerError(
          "Unable to process this transaction in xdc node, something wrong with your transaction/smart contract call input",
          ErrorTypes.INVALID_TRANSACTION
        );
    }
  }
  return new ManagerError(
    "Error while processing, but no message found",
    ErrorTypes.INTERNAL_ERROR
  );
};
